...
- The `num_agent` parameter can be changed to anywhere between 1 and 4 agents. The first agent is the ego car, all further agents are opponents.
- The ego and opponent starting poses can also be changed via parameters (`sx`/`sy`/`stheta` for the ego, `sx1..3`/`sy1..3`/`stheta1..3` for the opponents), these are in the global map coordinate frame.
...
In addition to the topics available in the single agent scenario, these topics are also available for each opponent. Opponent namespaces and topics carry the opponent's index as a suffix — the first opponent has no suffix (identical to the old two-agent setup), the second and third use `2` and `3`:
